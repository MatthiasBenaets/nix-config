...
| *Editor:*    | Nvim + Doom Emacs | Nvim + Doom Emacs |
| *Launcher:*  | Rofi              | /                 |
| *GTK Theme:* | Dracula           | /                 |
...
1. If you are planning to use doom emacs. It's recommended to comment out the import in configuration.nix for the initial installation if no user environment is already available.
   - This is due to the post install script is partially hardcoded
   - If you move your flake after first install, it's no longer relevant
   - The installation script is hardcoded for ~$HOME/.setup~, so save the flake there!
